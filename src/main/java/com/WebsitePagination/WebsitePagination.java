...
public class WebsitePagination {
	...
	public static void main(String a[]) {
	  ...
	  public static List<String> fetchItemsToDisplay(List<List<String>> items, int sortParameter, int sortOrder, int itemPerPage, int pageNumber) { 
		  ...
		  if(pageNumber > items.size() / itemPerPage || sortParameter > 2 || sortOrder > 1)     
			  return null;        
		  
		  items = sortItems(items, sortParameter, sortOrder);  
		  List<String> result = new ArrayList<String>();      
		  for(int i = 0; i < itemPerPage && ((pageNumber * itemPerPage + i) < items.size()); i++)   
			  result.add(items.get(pageNumber * itemPerPage + i).get(0));   
		  return result;   
	  }
	  private static List<List<String>> sortItems(List<List<String>> items, int sortParameter, int sortOrder) { 
